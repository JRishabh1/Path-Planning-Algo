...
def RRT(image, node_list, potential_map, boundary, end, RRTIterations, laplaceIterations, step_size, file_name, scaleDownFactor, start_time, result_images, times, distances, bpl):
    ...
    while True:
        ...
        for j in range(laplaceIterations): 
            ...
            # TRY USING METHODS INSTEAD OF FOR-LOOPS TO MAKE FASTER!!
        ...
        test_im = Image.open(file_name)
        w, h = test_im.size

        test_im = test_im.resize((round(w / scaleDownFactor), round(h / scaleDownFactor))) # just to make the image smaller
        testResult = test_im.copy()

        tw, th = testResult.size
        ...
        for ti in range(tw):
            for tj in range(th):
                ...
                testResult.putpixel((ti, tj), (tgs, tgs, tgs)) # PIL Image

        # Go from PIL to CV2
        testResult2 = cv2.cvtColor(np.array(testResult), cv2.COLOR_RGB2BGR)

        edge = cv2.Canny(testResult2, 50, 150) 
        
        # Go from CV2 to PIL
        edge = Image.fromarray(cv2.cvtColor(edge, cv2.COLOR_BGR2RGB))

        # Get array of edges
        edgeArray = np.asarray(edge)
        edges = []
        eh = len(edgeArray)
        el = len(edgeArray[0])

        # NUMPY.WHERE() CAN BE FASTER HERE!!!!
        # .argwhere()?
        for ei in range(1, el-2):
            for ej in range(1, eh-2):
                if(edgeArray[ej][ei][0] >= 240):
                    edges.append(Node(ei, ej))
        ...
        # for vi in range(vw):
        #     for vj in range(vh):
        #         gs = 255 - round(255 * map[vj][vi])
        #         if(map[vj][vi] != 1 and gs < 50):
        #             gs = 50
        #         videoResult.putpixel((vi, vj), (gs, gs, gs))
        videoResult = testResult.copy()
        for b in range(len(boundary)):
