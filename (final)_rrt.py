...
def random_point(image, height, length, end):
    check = True
    new_x = -1
    new_y = -1
...
# RRT Algorithm
def RRT(image, node_list, start, end, iterations, step_size, file_name, line_list, times, distances, start_time, result_images, no_draw):
    height = len(image)
    length = len(image[0])
    ...
    while True:
        ...
        # Get random point
        new_x, new_y = random_point(image, height, length, end)
        ...
        if check3 == True:
            ...
            if nodeCon:
                ...
                if end[0] - 2 < tx and end[0] + 2 > tx and end[1] - 2 < ty and end[1] + 2 > ty:
...
def RRTOriginalFunction(image, start, end, iterations, step_size, output_folder, output_path, fps, 
                        output_image, output_plot, data_file, parameter_file, no_draw):
    ...
    parent_array = RRT(image, node_list, start, end, iterations, step_size, file_name, line_list, times, distances, startTime, result_images, no_draw)
...
def main():
    ...
    output_folder = 'may21_videos'
    fps = int(30)

    no_draw = True # False means draw
    ...
    for i in range(len(images)):
        for j in range(len(iterations)):
            for k in range(len(step_sizes)):
                ...
                RRTOriginalFunction(images[i] + '.png', start, end, iterations[j], step_sizes[k], output_folder, output_path, fps, 
                                    output_image, output_plot, data_file, parameter_file, no_draw)
