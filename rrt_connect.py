...
def RRT_Connect(image, start, end, iterations, step_size, node_list1, node_list2):
    ...
    while True:
        ...
        # Maybe get nearest_node instead of just growing both trees like this then checking collision
        ...
        nearest3_ind = nearest_node(tx1, ty1, node_list2)
        nearest3_x = node_list2[nearest3_ind].x
        nearest3_y = node_list2[nearest3_ind].y
        connection = not collision(image, tx1, ty1, nearest3_x, nearest3_y)

        if connection:
            if nearest3_x-step_size+1 < tx1 and tx1 < nearest3_x+step_size-1 and nearest3_y-step_size+1 < ty1 and ty1 < nearest3_y+step_size-1:
                print("Path has been found after " + str(total_iter) + " iterations!")
                return node_list1[i1-1].parent_x, node_list1[i1-1].parent_y, node_list2[nearest3_ind].parent_x, node_list2[nearest3_ind].parent_y

# Draw the RRT nodes and path
def draw_result(image, result, start, end, parent_x_array1, parent_y_array1, parent_x_array2, parent_y_array2, step_size, node_list1, node_list2):
...
def main():
    ...
    parent_x_array1, parent_y_array1, parent_x_array2, parent_y_array2 = RRT_Connect(image, start, end, iterations, step_size, node_list1, node_list2)
    draw_result(image, result, start, end, parent_x_array1, parent_y_array1, parent_x_array2, parent_y_array2, step_size, node_list1, node_list2)
